...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  // ── Detail modal gig ──────────────────────────────────────
  window.openGigDetail = function (gigId) {
    const g = _gigsData.find(x => x.id === gigId);
    if (!g) return;

    const going    = _gigGoingSet.has(g.id);
    const dateStr  = formatGigDate(g.date);
    const lineupHtml = g.lineup.map(l => `
      <div class="flex items-center gap-2 py-2 border-b border-[#1a1a28] last:border-0">
        <span class="text-lg">${l.cover}</span>
        <span class="text-sm text-white font-medium">${l.name}</span>
        ${l.verified ? '<i class="fa-solid fa-circle-check text-brand text-xs ml-auto"></i>' : ''}
      </div>`).join('');

    // Gunakan share modal sebagai detail modal — inject konten
    const modal = document.getElementById('submitGigModal');
    modal.innerHTML = `
      <div class="modal max-w-lg w-full" style="max-height:90vh;overflow-y:auto;" onclick="event.stopPropagation()">
        <!-- Poster header -->
        <div class="relative h-36 bg-gradient-to-br from-brand/25 via-purple-900/20 to-black rounded-t-2xl flex items-center justify-center">
          <span class="text-8xl opacity-70">${g.poster}</span>
          <button onclick="closeSubmitGig()" class="absolute top-4 right-4 w-8 h-8 rounded-xl bg-black/50 flex items-center justify-center text-gray-300 hover:text-white transition">
        ...
        <div class="p-6">
          ...
          <div class="flex items-center gap-2 mb-3 flex-wrap">
            <span class="text-xs font-bold bg-brand text-white px-2.5 py-1 rounded-full flex items-center gap-1.5">
              <i class="fa-solid fa-calendar-day text-[10px]"></i> ${dateStr} · ${g.timeStart}
            </span>
            ${g.isFree ? '<span class="text-xs font-bold bg-green-500/20 text-green-400 border border-green-500/30 px-2.5 py-1 rounded-full">GRATIS</span>' : ''}
            ${g.genre ? `<span class="text-xs bg-white/5 text-gray-400 px-2.5 py-1 rounded-full">${g.genre}</span>` : ''}
          </div>

          <h2 class="text-xl font-bold text-white mb-1">${g.title}</h2>

          <div class="flex items-center gap-1.5 text-sm text-gray-400 mb-4">
            <i class="fa-solid fa-location-dot text-gray-600 text-xs"></i>
            ${g.venue}, ${g.kota}${g.provinsi ? ', ' + g.provinsi : ''}
          </div>

          ${g.description ? `<p class="text-sm text-gray-400 leading-relaxed mb-4">${g.description}</p>` : ''}

          <!-- Lineup -->
          ${g.lineup.length ? `
          <div class="mb-4">
            <div class="text-xs text-gray-500 font-semibold uppercase tracking-wider mb-2">Lineup</div>
            <div class="bg-black/20 rounded-xl px-4 py-1">${lineupHtml}</div>
          </div>` : ''}

          <!-- Harga & going -->
          <div class="flex items-center justify-between bg-black/20 rounded-xl px-4 py-3 mb-5">
            <div>
              <div class="text-xs text-gray-500 mb-0.5">Harga tiket</div>
              <div class="text-sm font-semibold ${g.isFree ? 'text-green-400' : 'text-white'}">
                ${g.isFree ? 'Gratis' : fmtPrice(g.priceMin, g.priceMax)}
              </div>
            </div>
            <div class="text-right">
              <div class="text-xs text-gray-500 mb-0.5">Akan hadir</div>
              <div class="text-sm font-semibold text-white flex items-center gap-1.5">
                <i class="fa-solid fa-user-group text-gray-500 text-xs"></i>
                <span id="detail-going-${g.id}">${g.goingCount}</span> orang
              </div>
            </div>
          </div>
          ...
          <div class="flex gap-3">
            <button onclick="toggleGoing('${g.id}', this)"
              class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl border text-sm font-semibold transition
                ${going ? 'bg-brand/15 border-brand/40 text-brand' : 'bg-white/5 border-white/10 text-gray-300 hover:border-brand/30 hover:text-brand'}">
              <i class="fa-${going ? 'solid' : 'regular'} fa-calendar-check"></i>
              <span id="going-label-${g.id}">${going ? 'Kamu hadir' : 'Tandai Hadir'}</span>
            </button>
            ${g.ticketUrl ? `
            <a href="${g.ticketUrl}" target="_blank" rel="noopener"
              class="flex-1 flex items-center justify-center gap-2 py-3 rounded-xl bg-brand hover:opacity-90 text-white text-sm font-semibold transition">
              <i class="fa-solid fa-ticket"></i> Beli Tiket
            </a>` : ''}
          </div>
        </div>
    ...
    document.getElementById('submitGigModal').classList.remove('hidden');
  };

  // ── Submit gig form ───────────────────────────────────────
  window.closeSubmitGig = function (e) {
    if (e && document.querySelector('#submitGigModal .modal')?.contains(e.target)) return;
    document.getElementById('submitGigModal').classList.add('hidden');
  };
